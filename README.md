...
Synthetic CI metrics from a real `make train-ci` run (2026-09-14). **n_volumes=6**, shape `8×64×64`, voxel `(40, 4, 4)` nm, 8 epochs, leakage_ok=true. Simulator scores only. If this table disagrees with [`reports/metrics.example.json`](reports/metrics.example.json) / a fresh `reports/metrics.json`, **trust the JSON**.
...
1. `make generate` — synthetic ssEM-like cubes (`8×64×64`). Vesicles are 40–60 nm blobs; mitochondria-like ovals are unlabeled distractors.
2. `make demo` — residual 3D U-Net + threshold baseline. Split is **by `volume_id`**, with raw-hash leakage checks. Inference uses **overlapping chunks** and writes only the inner region to avoid tile-seam double-counting.
3. Open `reports/overlay_panel.png` — raw | GT | threshold | U-Net.
4. Open `reports/metrics.json` — Dice, F1, IoU, density / µm³ (**simulator scores only**).
5. Optional: `make serve` → `http://127.0.0.1:8765/viewer/` (local process, not a public host).
